...
#Have been took values of requirement for solving problem.
deciders=input("How many there is Decider? (D): ")
criteria=input("How many there is Criteria? (C): ")
alternative=input("How many there is Alternative? (A): ")
deciders=int(deciders)
...
#Have been took values of deciders by criteria one by one from user.
for i in range(deciders):
    for j in range(criteria):
        a=input("Value of D"+str(i+1)+"-C"+str(j+1)+" (vl-l-ml-m-mh-h-vh): ")
        matrixOfImportantCriteria[i][j]=a
...
for i in range(alternative):
    for j in range(criteria):
        for k in range(deciders):
            b=input("Value of A"+str(i+1)+"-D"+str(k+1)+"-C"+str(j+1)+" (vp-p-mp-f-mg-g-vg): ")
            matrixOfImportantAlternative[i][j][k]=b
